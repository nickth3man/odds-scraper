...
def ev_calculator() -> None:
    ...
    with ui.column().classes('w-full p-6 gap-6'):
        ...
        def calculate() -> None:
            ...
            prob_value = prob_input.value
            odds_value = odds_input.value
            stake_value = stake_input.value
            if prob_value is None or odds_value is None or stake_value is None:
                error_label.text = 'Please fill in all fields with valid numbers.'
                return

            try:
                model_prob = float(prob_value) / 100
